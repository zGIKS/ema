...
class QThresholdPolicyTrainer:
    ...
    def fit(self, pos_sims: np.ndarray, neg_sims: np.ndarray, base_threshold: float,
            episodes: int = 2000, alpha: float = 0.2, gamma: float = 0.95,
            epsilon: float = 0.1, seed: int = 0) -> QPolicyResult:
        rng = np.random.default_rng(seed)
        policy = QLearningThresholdPolicy(n_states=self.n_bins, n_actions=len(self.actions),
        ...
        return QPolicyResult(q_table=policy.q.tolist(),
