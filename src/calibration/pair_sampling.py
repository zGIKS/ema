...
def sample_similarity_pairs(embeddings_store: dict, max_pairs: int = 2000, seed: int = 0) -> tuple[np.ndarray, np.ndarray]:
    """
    Build positive/negative cosine-similarity samples from stored embeddings.

    embeddings_store supported formats:
      - v1: {"name": [emb1, emb2, ...], ...}
      - v2: {"people": {"name": {"embeddings":[...], "prototype":[...]}, ...}}
    """
    ...
    # Positive: same person different embeddings (if only one, skip)
    for name in names:
